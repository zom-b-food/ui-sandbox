(function (a) {
    "use strict";
    var b = String.prototype.trim, c = String.prototype.trimRight, d = String.prototype.trimLeft, e = function (a) {
        return a * 1 || 0
    }, f = function (a, b, c) {
        a += "", b = ~~b;
        for (var d = []; b > 0; d[--b] = a);
    ...
    }, i = {lt: "<", gt: ">", quot: '"', apos: "'", amp: "&"}, j = {};
    for (var k in i)j[i[k]] = k;
    var l = function () {
        function a(a) {
        ...
        return c.format = function (c, d) {
            var e = 1, f = c.length, g = "", h, i = [], j, k, m, n, o, p;
            for (j = 0; j < f; j++) {
                g = a(c[j]);
                if (g === "string")i.push(c[j]); else if (g === "array") {
                    m = c[j];
                    if (m[2]) {
                        h = d[e];
                        for (k = 0; k < m[2].length; k++) {
                            if (!h.hasOwnProperty(m[2][k]))throw new Error(l('[_.sprintf] property "%s" does not exist', m[2][k]));
                            h = h[m[2][k]]
                        }
                    } else m[1] ? h = d[m[1]] : h = d[e++];
                    if (/[^s]/.test(m[8]) && a(h) != "number")throw new Error(l("[_.sprintf] expecting number but found %s", a(h)));
                    switch (m[8]) {
                        case"b":
                            h = h.toString(2);
                            break;
                        ...
                        case"d":
                            ...
                            break;
                        case"e":
                            h = m[7] ? h.toExponential(m[7]) : h.toExponential();
                            break;
                        case"f":
                            h = m[7] ? parseFloat(h).toFixed(m[7]) : parseFloat(h);
                            break;
                        case"o":
                            h = h.toString(8);
                            break;
                        case"s":
                            h = (h = String(h)) && m[7] ? h.substring(0, m[7]) : h;
                            break;
                        case"u":
                            h = Math.abs(h);
                            break;
                    ...
                    h = /[def]/.test(m[8]) && m[3] && h >= 0 ? "+" + h : h, o = m[4] ? m[4] == "0" ? "0" : m[4].charAt(1) : " ", p = m[6] - String(h).length, n = m[6] ? b(o, p) : "", i.push(m[5] ? h + n : n + h)
                }
            }
            return i.join("")
    ...
    }(), m = {VERSION: "2.1.1", isBlank: function (a) {
        return/^\s*$/.test(a)
        ...
        return("" + a).replace(/\&([^;]+);/g, function (a, b) {
            var c;
            return b in i ? i[b] : (c = b.match(/^#x([\da-fA-F]+)$/)) ? String.fromCharCode(parseInt(c[1], 16)) : (c = b.match(/^#(\d+)$/)) ? String.fromCharCode(~~c[1]) : a
        })
    ...
    }, sprintf: l, vsprintf: function (a, b) {
        ...
        var b = "ąàáäâãćęèéëêìíïîłńòóöôõùúüûñçżź", c = "aaaaaaceeeeeiiiilnooooouuuunczz", d = new RegExp(h(b), "g");
        ...
        var a = {};
        for (var b in this) {
            if (!this.hasOwnProperty(b) || ~m.words("include contains reverse").indexOf(b))continue;
            a[b] = this[b]
        }
        return a
    }, repeat: f};
    m.strip = m.trim, m.lstrip = m.ltrim, m.rstrip = m.rtrim, m.center = m.lrpad, m.rjust = m.lpad, m.ljust = m.rpad, m.contains = m.include, typeof exports != "undefined" ? (typeof module != "undefined" && module.exports && (module.exports = m), exports._s = m) : typeof define == "function" && define.amd ? define("underscore.string", function () {
        return m

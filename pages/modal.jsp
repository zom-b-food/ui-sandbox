	...
	<table class="uk-table">
		...
		<tbody>
			<tr>
				...
				<td>Add this class to a <code>&lt;div&gt;</code> element to create the dialog container and an overlay
					that blanks out the page. It is important to add an id to indicate the element for toggling.
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>Add this class to an <code>&lt;a&gt;</code> or <code>&lt;button&gt;</code> element to create a close
					button within the dialog box. We recommend adding the <code>.uk-close</code> class from the <a
							href="close.html">Close component</a> to give the button a proper styling, though you can
					also use text or an image.
				</td>
			</tr>
		</tbody>
	</table>
</div>
<p>You can use any element to toggle a modal dialog. An <code>&lt;a&gt;</code> element needs to be linked to the modal's
	id. To enable the necessary JavaScript, add the <code>data-uk-modal</code> attribute. If you are using another
	element, like a button, just add the <code>data-uk-modal="{target:'#ID'}"</code> attribute to target the modal's id.
</p>
...
<p>By default, the modal closes automatically when clicking on the modal overlay. To prevent this from happening, just
	add the <code>data-uk-modal="{target:'#ID',bgclose:false}"</code> attribute.</p>

<p>You can trigger a new modal inside the current modal. The default behaviour closes the first modal when a second one
	opens. You can prevent this from happening by adding the <code>data-uk-modal="{target:'#ID',modal:false}"</code>
	attribute.</p>
<hr class="uk-article-divider">
<h2 id="modal-header-footer"><a href="#modal-header-footer" class="uk-link-reset">Modal header and footer</a></h2>

<p>You can create a header and footer for your modal, which are seperated from the content. Just add the <code>.uk-modal-header</code>
	or the <code>.uk-modal-footer</code> class to a <code>&lt;div&gt;</code> element inside the modal dialog.</p>
...
<p>You can also create a caption that will be placed outside the modal. Just add the <code>.uk-modal-caption</code>
	class to a <code>&lt;div&gt;</code> element inside the modal dialog.</p>
...
<p>To create a lightbox-like modal dialog, just add the <code>.uk-modal-dialog-lightbox</code> class. This can be
	useful, if you want to use the modal as a lightbox for your images. The close button will adjust its position
	automatically to the dialog.</p>
...
<pre><code>&lt;!-- This is the anchor toggling the modal --&gt;
	...
	&lt;/div&gt;
	&lt;/div&gt;</code></pre>
<p><span class="uk-badge">NOTE</span> When creating a close button within the lightbox modifier, we also recommend
	adding the <code>.uk-close-alt</code> class from the <a href="close.html">Close component</a> to the close button to
	give your button a styling that fits the lightbox modal.</p>
<hr class="uk-article-divider">
<h2><a href="#modal-blank" class="uk-link-reset">Modal blank</a></h2>

<p>To reset all styling, like padding and margin, add the <code>.uk-modal-dialog-blank</code> class. This can be useful,
	if you want to create a fullscreen modal. In that case you also need to add the <code>.uk-height-viewport</code>
	class from the <a href="utility.html">Utility component</a> so that the modal extends to full viewport height.</p>
	...
	<div class="uk-modal-dialog uk-modal-dialog-blank">
		...
		<div class="uk-grid uk-flex-middle" data-uk-grid-margin>
			<div class="uk-width-medium-1-2 uk-height-viewport uk-cover-background"
			     style="background-image: url('images/placeholder_120x255.svg');"></div>
...
<p>To place a spinning icon inside your modal, add the <code>.uk-modal-spinner</code> class to a <code>
	&lt;div&gt;</code> element inside the modal dialog.</p>
...
<hr class="uk-article-divider">
<h2 id="overflow-container-in-modal"><a href="#overflow-container-in-modal" class="uk-link-reset">Overflow container in
	modal</a></h2>

<p>You can also display the modal's content in a scrollable container. Just add the <code>.uk-overflow-container</code>
	class to a <code>&lt;div&gt;</code> element inside the modal dialog. The modal will automatically expand and fill
	the site's height.</p>
...
<p>The modal component also provides alternatives to the native dialogs: window.alert, window.confirm and
	window.prompt.</p>
...
<button type="button" class="uk-button"
        onclick="UIkit.modal.confirm('Are you sure?', function(){ UIkit.modal.alert('Confirmed!'); });">Confirm
</button>
...
<button type="button" class="uk-button"
        onclick="UIkit.modal.prompt('Name:', '', function(val){ UIkit.modal.alert('Hello '+(val || 'Mr noname')+'!'); });">
	Prompt
</button>
...
<button type="button" class="uk-button"
        onclick="(function(modal){ modal = UIkit.modal.blockUI('Wait 5 sec...'); setTimeout(function(){ modal.hide() }, 5000) })();">
	Block UI
</button>
...
<p>The modal component triggers an <code>show.uk.modal</code> event every time a modal is opened and
	<code>hide.uk.modal</code> when a modal is closed.</p>

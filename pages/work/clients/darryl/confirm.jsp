	...
	<body>
		...
		<div id="container-page">
			...
			<div id="content">
				...
				<div id="inq">
					<c:choose>
						<c:when test="${searchList.rowCount != 0}"> <span
								class="head7">Sorry, no record found.</span> </c:when>
						<c:otherwise>
							<c:set var="noOfRows" value="1"/>
							<sql:query var="showList" dataSource="jdbc/demo"
							           sql="SELECT * FROM account ORDER BY companyName" startRow="${param.start}"
							           maxRows="${noOfRows}"/>
							<%-- Set number of rows to process --%>
							<c:forEach items="${showList.rows}" var="row">
								<input type="hidden" class="MEDformField" name="actKey" id="actKey"
								       value="<%= request.getParameter("actKey") %>">
								<input type="hidden" class="MEDformField" name="companyName" id="companyName"
								       value="<%= request.getParameter("companyName") %>">
								<input type="hidden" class="MEDformField" id="primFirstName" name="primFirstName"
								       value="<%= request.getParameter("primFirstName") %>"/>
								<input type="hidden" class="MEDformField" id="primLastName" name="primLastName"
								       value="<%= request.getParameter("primLastName") %>"/>
								<input type="hidden" class="MEDformField" id="primEmail" name="primEmail"
								       value="<%= request.getParameter("primEmail") %>"/>
								<input class="LONGformField" type="hidden" id="billAdd1" name="billAdd1"
								       value="<%= request.getParameter("billAdd1") %>"/>
								<input class="LONGformField" type="hidden" id="billAdd2" name="billAdd2"
								       value="<%= request.getParameter("billAdd2") %>"/>
								<input class="LONGformField" type="hidden" id="billCity" name="billCity"
								       value="<%= request.getParameter("billCity") %>"/>
								<input type="hidden" class="TINYformField" name="stringBillState" id="stringBillState"
								       value="<%= request.getParameter("stringBillState") %>"/>
								<input type="hidden" class="TINYformField" name="stringNeedData" id="stringNeedData"
								       value="<%= request.getParameter("stringNeedData") %>"/>
								<input class="LONGformField" type="hidden" id="billZip" name="billZip"
								       value="<%= request.getParameter("billZip") %>"/>
								<input class="LONGformField" type="hidden" id="primAC" name="primAC"
								       value="<%= request.getParameter("primAC") %>"/>
								<input class="LONGformField" type="hidden" id="primPhone" name="primPhone"
								       value="<%= request.getParameter("primPhone") %>"/>
								<input type="hidden" class="MEDformField" name="needData" id="needData"
								       value="<%= request.getParameter("needData") %>">
								<input type="hidden" class="MEDformField" name="curURL" id="curURL"
								       value="<%= request.getParameter("curURL") %>">
								<input type="hidden" class="MEDformField" id="notes" name="notes"
								       value="<%= request.getParameter("notes") %>"/>

								<form action="${contextPath}/servlet/authenticate.darrylEmail" name="activate2"
								      id="activate2" method="post">
									...
									<input id="subject" name="subject" type="hidden"
									       value="ui-design-engineering.com confirmation"/>
      ...
      <p style='font-family: verdana; font-size: 11px; color: #113b04'>Thanks,<br>
        ...
        am@ui-design-engineering.com </p></td>
			 ...
			 "/>
									<table cellpadding="0" cellspacing="0" border="0">
										<tr>
											<td align="left" style="padding-bottom:2px;">
												<strong>Welcome ${fn:escapeXml(param.companyName)}</strong></td>
										<tr>
									</table>
									...
									<br>
									<table width="550" border="0" cellpadding="1" cellspacing="0"
									       style="border:1px solid #95aa8e;">
										<th>Account Information</th>
										<tr>
											<td><span
													class="confirm">Company Name:</span><strong> ${fn:escapeXml(param.companyName)}</strong>
											</td>
										</tr>
										<tr>
											<td><span
													class="confirm">Primary Administrator:</span><strong> ${fn:escapeXml(param.primFirstName)}&nbsp;${fn:escapeXml(param.primLastName)}</strong>
											</td>
										</tr>
										<tr>
											<td><span
													class="confirm">Primary Email:</span><strong> ${fn:escapeXml(param.primEmail)}</strong>
											</td>
										</tr>
										<tr>
											<td><span
													class="confirm">Primary Phone:</span><strong> ${fn:escapeXml(param.primAC)}&nbsp;${fn:escapeXml(param.primPhone)}</strong>
											</td>
										</tr>
									</table>
									<br>
									<table width="550" border="0" cellpadding="1" cellspacing="0"
										...
										<tr>
											<td><span
													class="confirm">Street Address:</span><strong> ${fn:escapeXml(param.billAdd1)}&nbsp;${fn:escapeXml(param.billAdd2)}</strong>
											</td>
										</tr>
										<tr>
											<td><span
													class="confirm">City, State & Zip:</span><strong> ${fn:escapeXml(param.billCity)},&nbsp;${fn:escapeXml(param.billState)}&nbsp;${fn:escapeXml(param.billZip)}</strong>
											</td>
										</tr>
									</table>
									<br>

...
folder `angular-demo-snapshots`, which then contains an additional folder called `sandbox`.
...
- **snaphot.sh**: Run this script to generate the `angular-demo-snapshots` folder, containing
...
The critter data used by the application is scraped from a Google search page.

- ScrapeData.js: Run this script using node.js to access the web page containing the critter data and
  copy the data into the `app/critters` folder.

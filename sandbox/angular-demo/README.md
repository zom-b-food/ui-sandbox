# AngularJS critter Catalog Tutorial Application

## Overview

This application takes the developer through the process of building a web-application using
angular. The application is loosely based on the **Google critter Gallery**, which no longer
exists. Here is a historical reference: [Google critter Gallery on WayBack](http://web.archive.org/web/20131215082038/http://www.android.com/devices/).
...
- Add static html list with two critters into index.html. We will convert this static page into
...
- Convert the static html list into dynamic one by:
  - creating `critterListCtrl` controller for the application.
  ...
  - converting the static HTML document into an Angular template with the use of the `ngRepeat`
    directive which iterates over the dataset of critters.
...
- Add a search box to demonstrate how:
  ...
  - `ngRepeat` automatically shrinks and grows the number of critters in the view.
...
- Add `age` property to each critter in the data model.
- Add a `<select>` input to change the critter list order.
...
- Replace the in-memory dataset with data loaded from the server (in
  the form of static `critters.json` file).
  - The `critters.json` file is loaded using the `$http` service.
...
- Add critter images and links to new pages that show the critter details.
...
- Introduce the [$route] service which allows binding URLs for deep-linking with
  views:
  - Create `critterCatCtrl` which governs the entire app and contains $route configuration.
...
- Create critter list route:
  - Map `/critters` route to `critterListCtrl` and `partials/critters-list.html`.
  - Preserve existing `critterListCtrl` controller.
  - Move existing html from `index.html` to `partials/critter-list.html`.
- Create critter details route:
  - Map `/critters/<critter-id>` route to `critterDetailCtrl` and `partials/critters-detail.html`.
  - Create empty placeholder `critterDetailsCtrl` controller.


### step-8


- Implement `critterDetailCtrl` controller to fetch the details for a specific critter from a JSON file
  using `$http` service.
- Update the template for the critter detailed view.
- Add CSS to make the critter details page look "pretty".


### step-9

- Add custom `checkmark` filter.
- Update critter detail template to use `checkmark` filter.
- Add unit test for the filter.

### step-10

In the critter detail view, clicking on a thumbnail image, changes the main critter image to be the
large version of the thumbnail image.

- Define `mainImageUrl` model variable in the `critterDetailCtrl` and set its default value.
...
- Created a custom `critter` service that represents the `$resource` client.


### step-12

- Add animations to the application:
  - Animate changes to the critter list, adding, removing and reordering critters.
  - Animate changes to the main critter image in the detail view.


## Development with angular-demo

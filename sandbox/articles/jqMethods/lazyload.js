(function ($) {
    $.fn.lazyload = function (options) {
        ...
        if ("scroll" == settings.event) {
            $(settings - fluid).bind("scroll", function (event) {
                var counter = 0;
        ...
        $(settings - fluid).trigger(settings.event);
        return this;
    };
    $.belowthefold = function (element, settings) {
        if (settings - fluid === undefined || settings - fluid === window) {
            var fold = $(window).height() + $(window).scrollTop();
        }
        else {
            var fold = $(settings - fluid).offset().top + $(settings - fluid).height();
        }
    ...
    $.rightoffold = function (element, settings) {
        if (settings - fluid === undefined || settings - fluid === window) {
            ...
            var fold = $(settings - fluid).offset().left + $(settings - fluid).width();
        }
    ...
    $.abovethetop = function (element, settings) {
        if (settings - fluid === undefined || settings - fluid === window) {
            ...
            var fold = $(settings - fluid).offset().top;
        }
    ...
    $.leftofbegin = function (element, settings) {
        if (settings - fluid === undefined || settings - fluid === window) {
            ...
            var fold = $(settings - fluid).offset().left;
        }

var _bsap = {apiurl: 'http:\/\/s3.buysellads.com\/r\/', dz: [], dii: [], inv: [], drop: function (a, b) {
    ...
    bsa.filter(z.filterby, function (f) {
        ...
        var empty = 0, c = (!zf || !zf.ads || !zf.ads.length) ? [] : bsa.getads(zf, z.nads), d, e = c[0], t = '', o = '', fr, a, backfilled = false, tf = z.format ? z.format : (z.type && z.type == 1 ? 2 : 0), ts = Math.round(+new Date() / 1000), w = z.width, h = z.height, css = function (x) {
            var newel = document.createElement('style');
            newel.type = 'text\/css';
            newel.id = 'bsa_css';
            var ie6 = newel.styleSheet ? typeof(newel.styleSheet.cssText) == 'unknown' : false, el = !ie6 ? document.getElementById('bsa_css') || newel : newel;
            el.styleSheet ? (ie6 ? el.styleSheet.cssText = x : el.styleSheet.cssText += x) : el.appendChild(document.createTextNode(x));
            document.getElementsByTagName('head')[0].appendChild(el)
        };
        if (z.model == 1 && zf && zf.ads)for (empty = 100, i = 0; i < zf.ads.length; i++)empty -= zf.ads[i].per / z.nads;
        if (z.noempties && z.noempties == 1)empty = 0;
        if (t <= 1) {
            var ah = (typeof(ShowAdHereBanner) === 'object' ? ShowAdHereBanner[z.id] : z.showadhere) > 0, ra = (typeof(RepeatAll) === 'object' ? RepeatAll[z.id] : z.repeathere) > 0
        }
        if (tf == 0) {
            if (typeof(ShowAdHereBanner) === 'undefined' && !z.nostyle) {
                var bs = z.bannerstyles, sc = 'div.bsap_' + z.id + '{width:' + (z.vertical > 0 ? w + 'px' : '100%') + ';display:block}div.bsap_' + z.id + ' a{width:' + w + 'px}div.bsap_' + z.id + ' a img{padding:0}div.bsap_' + z.id + ' a em{font-style:normal}';
            ...
            for (i = 0, j = 0; i < c.length && (a = bsa.getads(c[i], 1)[0]); i++, (Math.random() * 100 < (z.model == 1 && z.nads > c.length ? (100 - c[i - 1].per) : empty)) || (j++, o += (a.flash && bsa.hasFlash() ? ('<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="' + z.width + '" height="' + z.height + '" id="bsa_flash"><param name="movie" value="' + a.flash + '"/><param name="menu" value="false"/><param name="quality" value="high"/><param name="wmode" value="opaque"/><param name="allowScriptAccess" value="always"/><param name="FlashVars" value="clickTag=' + bsa.tracker('click.go', a.id, z.id, '', '', '&link=' + a.link.replace('[timestamp]', ts)) + '"/><embed src="' + a.flash + '" width="' + z.width + '" height="' + z.height + '" quality="high" wmode="opaque" menu="false" allowScriptAccess="always" swliveconnect="true" flashvars="clickTag=' + bsa.tracker('click.go', a.id, z.id, '', '', '&link=' + a.link.replace('[timestamp]', ts)) + '" type="application/x-shockwave-flash"></embed></object>') : ('<a ' + bsa.link(a.id, z.id, a.link.replace('[timestamp]', ts)) + ' class="ad' + j + ' ' + (j % 2 === 0 ? 'even' : 'odd') + '" title="' + a.alt + '" id="bsa_' + a.id + '" target="_blank"><img src="' + a.img.replace('[timestamp]', ts) + '" width="' + z.width + '" height="' + z.height + '" alt="' + a.alt + '"\/><\/a>')), t += a.id + ';')) {
            }
            if (z.model == 1 && z.cpmbackfill && j == 0) {
            ...
            for (i = j; !backfilled && i < z.nads && ah && (i < (ra ? z.nads : (j + 1))); i++, o += (bsa.callback(z.id, i - 1, z.nads) || '<a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=adhere&utm_content=zone_' + z.id + '" title="Advertise Here" class="adhere ad' + i + ' ' + (i % 2 === 0 ? 'even' : 'odd') + '" target="_blank">Advertise Here<\/a>')) {
            }
            if (z.model == 1 && ah && (j == z.nads || backfilled))o += '<div class="bsap_adhere' + (w == 728 ? '2' : '') + '"><a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=cpmadhere&utm_content=zone_' + z.id + '">advertise here<\/a><\/div>'
        } else if (tf == 1) {
            ((z.model == 1 && Math.random() * 100 * z.nads < (empty || 0)) || c.length == 0) && ah && (c = [
                {id: -i, link: 'http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id, img: bsa.apiurl + 'd\/adhere.png', alt: 'Advertise Here', text: 'Advertise in this spot with Online Advertising Marketplace'}
            ]);
            ...
            if (c.length > 0)o += '<span class="bsa_it_p"><a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=imagetext&utm_content=zone_' + z.id + '">ads by BSA<\/a>' + (z.carrousel ? '<br/><a href="#" class="prevad" onclick="_bsap.inv[' + z.id + ']()">&larr;<\/a> <a href="#" class="nextad" onclick="_bsap.inv[' + z.id + ']()">&rarr;<\/a>' : '') + '<\/span><\/div>'
        } else if (tf == 2) {
            c.length == 0 && ah && c.push({id: -i, link: 'http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '\/zone\/' + z.id, title: 'Advertise Here', text: 'Advertise in this spot with Online Advertising Marketplace'});
            ...
            o += '<\/ul>';
            if (c.length > 0)o += '<div class="bsa_idb"><span class="bsa_idl"><a href="http:\/\/buysellads.com\/buy\/detail\/' + z.siteid + '?utm_source=site_' + z.siteid + '&utm_medium=website&utm_campaign=textdesc&utm_content=zone_' + z.id + '" target="_blank">ads by <em>BSA<\/em><\/a><\/span><\/div><\/div><\/div>'
        } else if (tf == 4) {
            ...
            if (a && (Math.random() * 100 > (z.model == 1 && z.nads > c.length ? (100 - c[0].per) : empty))) {
                ...
                b.onclick = function (e) {
                    if (e.target.getAttribute('id') == ('bsap_' + z.id))window.location = bsa.tracker('click.go', a.id, z.id, '', '', '&link=' + a.link.replace('[timestamp]', ts))
                };
                t += a.id
            }
        }
        if (o && !b.innerHTML.length)b.innerHTML = o;
    ...
    return'http:\/\/stats.buysellads.com\/' + t + '?z=' + z + '&b=' + b + '&g=' + u + '&s=' + s + '&sw=' + screen.width + '&sh=' + screen.height + '&br=' + this.br() + '&r=' + Math.random() + additional
}, br: function () {
    var a = navigator.userAgent, p = navigator.platform, m = function (r, h) {
        for (var i = 0; i < h.length; i++)r = r.replace(h[i][0], h[i][1]);
        return r
    }, i = (a.match(/Opera|Navigator|Minefield|KHTML|Chrome/) ? m(a, [

...
copyProperties(EmuTracker.prototype, {sendData: function (a, b) {
    this.base.event(a, b);
}});
...
__d("Button", ["event-extensions", "CSS", "DataStore", "DOM", "Parent"], function (a, b, c, d, e, f) {
    ...
    var s = {getInputElement: function (t) {
        ...
        return !g.hasClass(q(t), k);
    }, setEnabled: function (t, u) {
        t = q(t);
    ...
    }, setDepressed: function (t, u) {
        g.conditionClass(q(t), l, u);
    }, isDepressed: function (t) {
        return g.hasClass(q(t), l);
    }, setLabel: function (t, u) {
        t = q(t);
        if (r(t)) {
...
__d("Form", ["event-extensions", "AsyncRequest", "AsyncResponse", "CSS", "DOM", "DOMPosition", "DOMQuery", "DataStore", "Env", "Input", "Parent", "URI", "createArrayFrom", "trackReferrer"], function (a, b, c, d, e, f) {
    ...
    var w = {getInputs: function (x) {
        ...
        return r(x.tagName == 'FORM' ? x.elements : w.getInputs(x));
        ...
        s(x, da);
        var ea = new g(da);
        ea.setData(ca).setNectarModuleDataSafe(x).setReadOnly(z == 'GET').setMethod(z).setRelativeTo(x).setStatusElement(aa).setInitialHandler(w.setDisabled.curry(x, false)).setHandler(function (fa) {
            Event.fire(x, 'success', {response: fa});
        }).setErrorHandler(function (fa) {
            if (Event.fire(x, 'error', {response: fa}) !== false)h.defaultErrorHandler(fa);
        }).setFinallyHandler(w.setDisabled.curry(x, false)).send();
    }, forEachValue: function (x, y, z) {
        w.getElements(x).forEach(function (aa) {
            if (aa.name && !aa.disabled && aa.type !== 'submit')if (!aa.type || ((aa.type === 'radio' || aa.type === 'checkbox') && aa.checked) || aa.type === 'text' || aa.type === 'password' || aa.type === 'hidden' || aa.nodeName === 'TEXTAREA') {
                z(aa.type, aa.name, o.getValue(aa));
...
__d("JSONPTransport", ["ArbiterMixin", "DOM", "HTML", "URI", "copyProperties"], function (a, b, c, d, e, f) {
    ...
    k(q.prototype, g, {getID: function () {
        return this._id || (this._id = m++);
    }, hasFinished: function () {
        return !(this.getID() in l);
    }, getRequestURI: function () {
        return j(this._uri).addQueryData({__a: 1, __adt: this.getID()});
...
__d("Animation", ["CSS", "DOM", "Style", "UserAgent"], function (a, b, c, d, e, f) {
    ...
    k.prototype.to = function (l, m) {
        if (m === undefined) {
            this._attr(this.last_attr, l, k.ATTR_TO);
        }
        else {
    ...
    k.prototype.by = function (l, m) {
        if (m === undefined) {
            this._attr(this.last_attr, l, k.ATTR_BY);
        }
        else {
    ...
    k.prototype.from = function (l, m) {
        if (m === undefined) {
            this._attr(this.last_attr, l, k.ATTR_FROM);
        }
        else {
            ...
            for (var t in s.attrs)switch (t) {
                ...
                case 'color':
                    if (s.attrs[t].start[3] != s.attrs[t].value[3]) {
                        this.obj.style[t] = 'rgba(' + k.calc_tween(v, s.attrs[t].start[0], s.attrs[t].value[0], true) + ',' + k.calc_tween(v, s.attrs[t].start[1], s.attrs[t].value[1], true) + ',' + k.calc_tween(v, s.attrs[t].start[2], s.attrs[t].value[2], true) + ',' + k.calc_tween(v, s.attrs[t].start[3], s.attrs[t].value[3], false) + ')';
                    }
    ...
    k.parse_color = function (l) {
        ...
        if (m) {
            return [parseInt(m[1].length == 1 ? m[1] + m[1] : m[1], 16), parseInt(m[2].length == 1 ? m[2] + m[2] : m[2], 16), parseInt(m[3].length == 1 ? m[3] + m[3] : m[3], 16), 1];
        }
        else {
            var n = /^rgba? *\(([0-9]+), *([0-9]+), *([0-9]+)(?:, *([0-9]+))?\)$/.exec(l);
            if (n) {
                return [parseInt(n[1], 10), parseInt(n[2], 10), parseInt(n[3], 10), n[4] ? parseFloat(n[4]) : 1];
            }
    ...
    k._animate = function (l, m) {
        l = l || Date.now();
        for (var n = (m === true) ? k.active.length - 1 : 0; n < k.active.length; n++)try {
            if (!k.active[n]._frame(l))k.active.splice(n--, 1);
        }
        catch (o) {
            k.active.splice(n--, 1);
        }
...
__d("Vector", ["event-extensions", "DOMDimensions", "DOMPosition", "copyProperties"], function (a, b, c, d, e, f) {
    ...
    function j(k, l, m) {
        i(this, {x: parseFloat(k), y: parseFloat(l), domain: m || 'pure'});
    }
...
__d("Dialog", ["array-extensions", "event-extensions", "Animation", "Arbiter", "AsyncRequest", "Bootloader", "Button", "CSS", "CSSSupport", "ContextualThing", "DOM", "Form", "HTML", "Input", "Keys", "Locale", "Parent", "Run", "Style", "URI", "UserAgent", "Vector", "bind", "copyProperties", "createArrayFrom", "getObjectValues", "getOverlayZIndex", "tx"], function (a, b, c, d, e, f) {
    ...
    ba(ha, {OK: {name: 'ok', label: "Okay"}, CANCEL: {name: 'cancel', label: "Cancel", className: 'inputaux'}, CLOSE: {name: 'close', label: "Close"}, NEXT: {name: 'next', label: "Next"}, SAVE: {name: 'save', label: "Save"}, SUBMIT: {name: 'submit', label: "Submit"}, CONFIRM: {name: 'confirm', label: "Confirm"}, DELETE: {name: 'delete', label: "Delete"}, _globalCount: 0, _bottoms: [0], max_bottom: 0, _updateMaxBottom: function () {
        ha.max_bottom = Math.max.apply(Math, ha._bottoms);
    }});
    ba(ha, {OK_AND_CANCEL: [ha.OK, ha.CANCEL], _STANDARD_BUTTONS: [ha.OK, ha.CANCEL, ha.CLOSE, ha.SAVE, ha.SUBMIT, ha.CONFIRM, ha.DELETE], SIZE: {WIDE: 555, STANDARD: 445}, _HALO_WIDTH: 10, _BORDER_WIDTH: 1, _PADDING_WIDTH: 10, _PAGE_MARGIN: 40, _stack: [], _isUsingCSSBorders: function () {
        return m.supportsBorderRadius() || y.ie() < 7;
    ...
    ba(ha.prototype, {_cross_quickling: false, _cross_transition: false, _loading: false, _showing: false, show: function () {
        ...
        var ja = o.scry(this._popup, 'input'), ka = function (la) {
            return la.name == ia.name;
        };
        ...
        if (ia) {
            if (this._showing) {
                this._autohide_timeout = setTimeout(this.hide.shield(this), ia);
        ...
        if (ja) {
            this.setHandler(this._submitForm.bind(this, 'POST', ia));
        ...
        if (ia) {
            ...
            this._semiModalListener = Event.listen(this._obj, 'click', function (ja) {
                if (!o.contains(this._popup, ja.getTarget()))this.hide();
            }.bind(this));
        ...
        for (var ia = 0; ia < this._onload_handlers.length; ++ia)try {
            this._onload_handlers[ia]();
        } catch (ja) {
        }
        this._onload_handlers = [];
        ...
        if (this._auto_focus) {
            ...
            if (ia) {
                p.focusFirst.bind(this, this._content).defer();
        ...
        if (y.ie() < 7 && !this._shim)j.loadModules(['IframeShim'], function (ua) {
        ...
        if (ha._isUsingCSSBorders()) {
            ia = '<div class="pop_container_advanced">' + '<div class="pop_content" id="pop_content"></div>' + '</div>';
        ...
        if (!this._popup)return;
        try {
            new g(this._obj).duration(0).checkpoint().to('opacity', 0).hide().duration(250).ondone(this._hide.bind(this, ia)).go();
        } catch (ja) {
            this._hide(ia);
        }
    }, _hide: function (ia) {
...
__d("DOMScroll", ["Animation", "Arbiter", "DOM", "DOMQuery", "UserAgent", "Vector", "ViewportBounds", "ge"], function (a, b, c, d, e, f) {
    var g = b('Animation'), h = b('Arbiter'), i = b('DOM'), j = b('DOMQuery'), k = b('UserAgent'), l = b('Vector'), m = b('ViewportBounds'), n = b('ge'), o = {SCROLL: 'dom-scroll', getScrollState: function () {
        ...
        if (r) {
            p.y -= l.getViewportDimensions().y / 2;
        } else if (s) {
        ...
        y += r;
        if (x < v) {
            o.scrollTo(new l(u, x, 'document'), s, false, false, t);
...
__d("LinkController", ["event-extensions", "DataStore", "Parent", "trackReferrer"], function (a, b, c, d, e, f) {
    ...
    var p = {registerHandler: function (q) {
        k.push(q);
...
__d("PageTransitions", ["event-extensions", "Arbiter", "Dialog", "DOM", "DOMScroll", "Env", "Form", "HistoryManager", "JSLogger", "LinkController", "OnloadHooks", "htmlize", "escapeJSQuotes", "URI", "UserAgent", "Vector", "areObjectsEqual", "clickRefAction", "computeRelativeURI", "copyProperties", "ge", "goOrReplace", "startsWith", "tx", "userAction"], function (a, b, c, d, e, f) {
    ...
    var ia = {_transition_handlers: [], _scroll_locked: false, isInitialized: function () {
        ...
        Event.listen(window, 'scroll', function () {
            if (!ia._scroll_locked)fa(ia._current_uri, u.getScrollPosition());
        });
        return ia;
        ...
        ia._loadPage(new s(ka), function (la) {
            if (!la)aa(window.location, ka, true);
        });
        return true;
        ...
        ia._current_uri = ia._most_recent_uri = ia._next_uri;
...
__d("LinkshimAsyncLink", ["$", "AsyncSignal"], function (a, b, c, d, e, f) {
    var g = b('$'), h = b('AsyncSignal'), i = {swap: function (j, k) {
        j.href = k;
...
__d("legacy:async-postlude", ["AsyncRequestNectarLogging"], function (a, b, c, d) {
    b('AsyncRequestNectarLogging');
}, 3);
...
__d("AjaxPipeRequest", ["Arbiter", "AsyncRequest", "BigPipe", "CSS", "DOM", "Env", "JSCC", "ScriptPathState", "URI", "copyProperties", "goOrReplace", "ge"], function (a, b, c, d, e, f) {
    ...
    p(v, {getCurrentRequest: function () {
        return s;
...
__d("requestAnimationFrame", [], function (a, b, c, d, e, f) {
    var g = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function (h) {
...
__d("PostLoadJS", ["Bootloader", "Run"], function (a, b, c, d, e, f) {
    ...
    function i(k, l) {
        h.onAfterLoad(function () {
            g.loadModules.call(g, [k], l);
        });
    }

    var j = {loadAndRequire: function (k) {
        i(k, bagofholding);
    }, loadAndCall: function (k, l, m) {
        i(k, function (n) {
            n[l].apply(n, m);
        });
    }};
...
copyProperties(FutureSideNav.prototype, {
    init: function (a, b, c) {
        ...
        if (this._pendingSections)this._pendingSections.forEach(function (d) {
        ...
        var b = function (c, d) {
            ...
            if (c.children)c.children.forEach(function (f) {
                b(f, e);
            });
        }.bind(this);
        ...
        var d = c.getLinkNode();
        d && this._eventHandlers.push(Event.listen(d, 'click', function (event) {
            return !this.editing;
        }.bind(this)));
        return c;
        ...
        if (!b) {
            ...
            this._handleMenuClick(a, c, b.parentNode, event);
        }
        ...
        if (CSS.hasClass(c, 'rearrange'))this.beginEdit(a);
        ...
        if (c)if (typeof b.count !== "undefined") {
            c.setCount(b.count, b.hide);
        }
...
copyProperties(FutureSideNavSection.prototype, {equals: function (a) {
...
__d("FormDisableOnSubmit", ["copyProperties"], function (a, b, c, d, e, f) {
    ...
    g(h.prototype, {_subscription: null, _disabled: false, enable: function () {
...
__d("FormSubmitOnChange", ["event-extensions", "copyProperties", "submitForm"], function (a, b, c, d, e, f) {
    ...
    g(i.prototype, {_listener: null, enable: function () {
        this._listener = Event.listen(this._form.getRoot(), 'change', this._submit.bind(this));
    }, disable: function () {
...
__d("UIForm", ["event-extensions", "ArbiterMixin", "BehaviorsMixin", "Class", "DOM", "Form", "FormDisableOnSubmit", "FormSubmitOnChange", "Run", "areObjectsEqual", "copyProperties"], function (a, b, c, d, e, f) {
    ...
    p(q.prototype, g, h, {getRoot: function () {
        return this._root;
...
__d("ChatConfig", ["copyProperties", "ChatConfigInitialData"], function (a, b, c, d, e, f) {
    var g = c('ChatConfigInitialData'), h = b('copyProperties'), i = {}, j = {get: function (k, l) {
        return k in i ? i[k] : l;
    }, set: function (k) {
        if (arguments.length > 1) {
            var l = {};
...
__d("ScrollingPager", ["Arbiter", "copyProperties", "CSS", "OnVisible", "UIPagelet", "$", "ge"], function (a, b, c, d, e, f) {
    ...
    h(o, {REGISTERED: 'ScrollingPager/registered', getInstance: function (p) {
        return n[p];
    }});
...
__d("TimelineController", ["event-extensions", "Arbiter", "CSS", "DataStore", "DOM", "Run", "ScrollingPager", "Style", "TimelineConstants", "TimelineURI", "Vector", "ViewportBounds", "$", "ge", "throttle"], function (a, b, c, d, e, f) {
    ...
    var za = {NAV: 'nav', STICKY_HEADER: 'sticky_header', STICKY_HEADER_NAV: 'sticky_header_nav', SCRUBBER: 'scrubber', CONTENT: 'content', ADS: 'ads', COVER: 'cover', LOGGING: 'logging', SECTION_LOADING: 'TimelineController/sectionLoading', SECTION_LOADED: 'TimelineController/sectionLoaded', SECTION_FULLY_LOADED: 'TimelineController/sectionFullyLoaded', SECTION_REGISTERED: 'TimelineController/sectionRegistered', SCRUBBER_DEFAULT_OFFSET: 38, init: function (ab, bb, cb) {
    ...
    }, setAdsTracking: function (ab) {
        ra(za.ADS, 'start', [ab]);
    }, coverHasExpanded: function (ab) {
        var bb = s('rightCol');
        if (bb) {
        ...
        g.inform('reflow');
    }, pageHasScrubber: function (ab) {
        return !ab || ab.match(/^(og_)?app_/) || (ab in na);
    }, fixedAds: function () {
        return ba;
    }, registerCurrentKey: function (ab) {
        y = ab;
        ...
        return ea[za.SCRUBBER];
    }, getCurrentStickyHeaderNav: function () {
        return ea[za.STICKY_HEADER_NAV];
    }, getSection: function (ab) {
        return ra(za.CONTENT, 'getSection', [ab]);
    }, scrubberHasLoaded: function (ab) {
        ...
        if (ea[za.STICKY_HEADER_NAV]) {
            ra(za.STICKY_HEADER_NAV, 'addTimePeriod', [ab]);
        } else oa[bb] = ab;
    }, scrubberHasChangedState: function () {
        ra(za.ADS, 'adjustAdsToFit');
    }, scrubberWasClicked: function (ab) {
        ra(za.LOGGING, 'logScrubberClick', [ab]);
    }, sectionHasChanged: function (ab, bb) {
    ...
    }, navigateToSection: function (ab) {
        ra(za.CONTENT, 'navigateToSection', [ab]);
        ...
        ra(za.STICKY_HEADER, 'adjustWidth');
    }, register: function (ab, bb) {
        ea[ab] = bb;
        if (fa[ab]) {
...
__d("TimelineStickyHeader", ["Arbiter", "CSS", "DOM", "TimelineController", "ViewportBounds"], function (a, b, c, d, e, f) {
    var g = b('Arbiter'), h = b('CSS'), i = b('DOM'), j = b('TimelineController'), k = b('ViewportBounds'), l = false, m = false, n, o, p, q, r = {VISIBLE: 'TimelineStickyHeader/visible', ADJUST_WIDTH: 'TimelineStickyHeader/adjustWidth', init: function (s) {
        ...
        q = k.addTop(function () {
            return m ? s.offsetTop + s.scrollHeight : 0;
        });
        ...
        g.inform(r.ADJUST_WIDTH, o, g.BEHAVIOR_STATE);
    }, getRoot: function () {
        return n;
